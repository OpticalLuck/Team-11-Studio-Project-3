...
void CSoundController::PlaySoundByID(const int ID)
{
	...
	{
		if (pSoundInfo->GetLoopStatus() == true)
		{
			snd = cSoundEngine->play2D(pSoundInfo->GetSound(),
...
void CSoundController::SetVolumeByID(const int ID, float volume)
{
	if (volume >= 1.f)
		volume = 1.f;
	else if (volume <= 0.f)
	...
	ISoundSource* pISoundSource = GetSound(ID)->GetSound();
	if(pISoundSource->getDefaultVolume() != volume)
		pISoundSource->setDefaultVolume(volume);
}

void CSoundController::UpdatePlayBackVolume(float volume)
{
	if (snd)
	{
